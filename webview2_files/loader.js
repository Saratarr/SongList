...
function init() {
	...
	function getResourceUrl() {
		if (isDebugLocalAssets()) {
			return 'http://127.0.0.1:9666/dashboardstyles/base';
		}
	...
	//  Load the CSS
	//
	eesyLoadCss(getCssUrl(getDashboardUrl(), '/static/css/proactiveresources-base.min.css'));
	...
	eesyLoadCss(getCssUrl(getDashboardUrl(), '/static/css/support-center-theme-defaults.min.css'));
	if (window.CANVAS_ACTIVE_BRAND_VARIABLES) {
		...
		eesyLoadCss(getCssUrl(getDashboardUrl(), '/static/css/support-center-theme-canvas.min.css'));
	}
